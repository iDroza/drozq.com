...
- **Dark mode email theme (Joshua, 2026-07-13: next pass).** The template currently pins `color-scheme: light`, so dark-mode clients render it light. ~70% of iOS users run dark mode; build the dual theme: `@media (prefers-color-scheme: dark)` styles (dark card on near-black backdrop, adjusted text/border tokens, logo swap to `brand-logo-white.png` via the same hotlink-proof route pattern), update the `color-scheme`/`supported-color-schemes` metas, and verify in Apple Mail + Gmail iOS (Gmail partially force-inverts light emails; test both).

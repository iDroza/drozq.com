...
- **Fello integration (keys in hand 2026-09-02, nothing wired yet).** Brief: `notes/fello/fello-api-brief.md`, rendered at `/fello/`; CLI `scripts/fello.py`. Build in this order once Joshua says go: (1) a best-effort `POST /contact` in `deliverLead` gated on a `FELLO_API_KEY` Pages env var (email, name, phone, `full_address`, tags for mode + timeline + origin page, `crmFields` pointing at the FUB person), (2) an HMAC-verified `/api/fello/webhook` receiver (`FELLO_CLIENT_SECRET`, 2xx under 5 s, work behind `waitUntil`): FormSubmission -> the lead pipeline, DashboardClick / EmailClick / PostcardScan / FelixAIHandoff -> hot alert + FUB tag, ContactUnsubscribed -> `emailer pause`, ContactEnriched / ContactDetailsUpdated -> FUB refresh; register with `python scripts/fello.py webhooks add-all https://drozq.com/api/fello/webhook`, (3) the tag vocabulary, (4) `leadScore` + engagement readback on the operating dashboard. Also: connect the native Fello <-> Follow Up Boss sync in Fello's App Marketplace with the PERSONAL FUB account owner key (that is what carries home values into FUB; the API never returns them).

...
- **Roll the homepage type scale to the sibling pages (needs Joshua's go).** 2026-09-03: the homepage's blocks below the splash were resized to the `#drozq-commit-strip` scale (TEMPLATE.md §5 "Homepage type scale": h2 `clamp(30px,3.4vw,46px)`, body 18/28 to 19/30, card copy 17/26 to 18/28, labels 12.5/13px) because Joshua found the old 14-16px copy hard to read. Every other template page still runs the old scale, so the site now has two type systems. When he approves, port the table as a scoped block per page (section ids differ), verify 375 / 768 / 1440 on each, and flip the TEMPLATE.md section to "sitewide". The `/process/` Five Steps block is the first candidate: the homepage copy of it already renders at the new sizes.
...
- **Fello: the values path.** The four integration steps shipped 2026-09-02 (push, webhook receiver, tag vocabulary, engagement readback; CLAUDE.md "Fello integration"). Still Joshua's clicks, not code: (1) in the Fello app connect Follow Up Boss (App Marketplace > Follow Up Boss) with the PERSONAL FUB owner API key and, in Field Mapping, map Fello's Home Value (AVM), Estimated Equity, Mortgage Balance, Lead Score and Intent attributes to new FUB custom fields (the "New" option creates them); `/api/fello/engagement` then surfaces them per lead automatically; (2) enable the Fello embedded app in FUB (Admin > Integrations > Embedded Apps) so the Fello card with AVM + equity shows on every contact; (3) optional: a Zapier "New Home Value View" trigger from the Fello app to a drozq webhook if the value ever needs to arrive as an event. Also: delete the throwaway `fello-api-probe@drozq.com` contact once the value mapping is verified (`python scripts/fello.py contact delete 4bf8dc0b-e1d5-4b2a-a68d-2b38fd48adbd --yes`).

...
- **Roll the full-screen hero + scroll-reveal header to the template pages (pending Joshua's approval).** The homepage runs the viewport-filling hero + scroll-revealed fixed header as of 2026-07-20 (homepage-only by instruction). Every other template page still renders the in-flow white header block because of the Panda `@layer base` `position:relative !important` trap (TEMPLATE.md §3). When approved: apply the same `@layer base` position override + scroll-reveal block per page, with page-appropriate hero imagery (a single image is fine; the Giem rotation stays a homepage signature).
- **Closing-CTA offer framing on 4 pages (Joshua's call).** `/field-notes/`, `/market-insights/`, `/prices/`, `/rates/` close with the human-CMA promise ("Free CMA, delivered within 24 hours" or equivalent) directly above forms that open the instant-valuation funnel. Decide per page: re-anchor to the instant valuation + 5 playbooks (like `/process/`'s closing, fixed 2026-07-14), or keep the hand-built-CMA angle as the closer. `/thank-you/` (reply-time commitment) and `/value/` (refined-CMA tier) are intentional and stay. The 2026-07-14 sweep already moved every landing CTA to "Run my Valuation" + the instant H4 line site-wide; only these four closer paragraphs remain on the old framing.

...
`/dashboard` carries a second dark row directly under the Google Ads row, mirroring it column for column: **SELLERS SPEND / LEADS / CPC / CPL**. It is the combined performance of the two seller search campaigns, `JT | Sell | OC | Search` (justintye.com/sell) and `AR | Sell | OC | Search` (activerealty.com/sell), which are one lander on two domains, so they are reported as one campaign. The window is **month to date** (since 2026-09-01; it was since-launch before that), clamped so it never starts before `GOOGLE_ADS_SELLER_CAMPAIGN_LAUNCH_DATE` (default `2026-08-17`, a non-secret var in `wrangler.jsonc`): in the launch month the cards print "Since Aug 17", in every later month "Month to date", exactly like the Google Ads row above. The snapshot exposes it as `sellerCampaignPeriod` (`getSellerCampaignPeriod` in `src/lib/date.ts`). `/active` does not receive these four metrics (they are omitted from `ACTIVE_METRIC_KEYS`).
...
Campaign selection (`isSellerCampaignName` in `src/sources/google-ads.ts`): with `GOOGLE_ADS_SELLER_CAMPAIGN_NAMES` set (comma-separated exact names, compared case- and whitespace-insensitively segment by segment) only those names count. Unset, the built-in rule accepts any `|`-delimited campaign name that carries a `JT` or `AR` segment, an `OC` segment, and a segment starting with `sell` (so `JT | Sell | OC | Search`, `AR | OC | Sellers`, and a future `AR | Sell | OC | Display` all qualify; `AR | Buy | OC | Search`, `JT Call Ads`, and `DSA General Site` do not). Spend is the exact `cost_micros` sum; Leads = the sum of primary `metrics.conversions`; CPC = spend / clicks; CPL = spend / leads. Both ratios fail closed on a zero denominator (impressions are still parsed for the log line but no CTR metric is published).

...
- [ ] `python scripts/panda_patch.py` (the no-op checker) exits clean. If the page uses a Panda utility the inherited soup never compiled, either use a compiled class, add a scoped `<style>` rule, or re-run `python scripts/panda_patch.py --apply` to fold it into the site-wide `<style id="drozq-panda-patch">` block (see §14).
...
**The `<main class="ov_hidden">` sticky trap (found 2026-08-01).** `position: sticky` resolves against the nearest ancestor with a **scrolling box**, and `overflow: hidden` creates one. The shared `<main class="ov_hidden">` is therefore the scrollport for anything inside it, so a sticky child silently never pins: it scrolls away with the page and looks like the property was ignored. Any page that wants a sticky element inside `<main>` must add `main.ov_hidden{overflow:visible}` to its own `<style id="drozq-page-chrome">` block (`/net-sheet/` does this for its net-sheet card) **and then verify there is no horizontal page scroll at 375px**, since `ov_hidden` was what clipped bleed. Wide content must already be inside its own `overflow-x: auto` wrapper. Related: never fix a too-tall sticky panel with `max-height` + `overflow-y: auto`; an internal scrollbar on a results card reads as broken. Let the card be its full height and drop it to `position: static` when it exceeds the viewport.
...
**Site-wide header variants (2026-07-20 redesign).** Every page now fixes the trap above via a `<style id="drozq-page-chrome">` block in `<head>` (the homepage's lives in `#drozq-hero-rotate-css`), in one of two variants:

- **Variant A (photo-splash pages: `/`, `/california/`, `/los-angeles/`, `/where-we-help/`):** fixed header, fully invisible at the top (`opacity:0; visibility:hidden; pointer-events:none`), fading in as the white bar + shadow past 64px of scroll (class `drozq-hdr-on`, toggled by the tiny `#drozq-hdr-reveal` script, or the homepage's rotate JS). The photo owns the first paint.
- **Variant B (every other page):** fixed, ALWAYS-visible white bar + shadow, with `body{padding-top:48px}` (64px at `md`) so content clears it. The nav follows the reader down long resource pages instead of scrolling away.
...
**Homepage (2026-07-20): full-screen rotating photo hero.** The homepage hero is viewport-filling (`min-height:100dvh`, `100vh` fallback). The splash (the hero `<img>`, `fetchpriority="high"`) is `/media/images/hero-giem/giem-01.webp`; a continuous one-way rotation then cycles the ring `giem-01..10.webp` + `crystal-cove.webp` forever. Joshua's allocation rule (2026-07-20): the ranked top 10 are homepage-exclusive; giem-11..20 are reserved for OTHER pages' heroes, and any of 11-20 left unassigned get appended back into the homepage ring: first swap at 1.5s (to giem-02), each following swap every 3.5s, 0.9s crossfades, no pause and no terminal state, no visitor controls (cannot move back or forward), `prefers-reduced-motion` skips the rotation entirely. Implemented by the `#drozq-hero-rotate-css` + `#drozq-hero-rotate-js` block at the end of `<body>` (outside the funnel markers, NOT synced): two absolutely positioned layers injected under `[data-testid="hero-tint"]`, so the 0.4 dark tint and the hero copy contrast survive every slide. One hard-won bug: when handing the finished slide from the fade layer to the base layer, suspend the base layer's opacity transition for the swap (`transition:'none'`, set, reflow, restore); letting it animate 0 to 1 re-exposes the splash image underneath for ~900ms and reads as a lag-jerk back to the first photo on the first swap. Homepage-only until Joshua approves the rollout.

**The page-archetype system (2026-07-20 redesign, per Joshua: pages must read as resources, not homepage photocopies).** `crystal-cove.webp` is now homepage-ring-only, and only true landers keep the tabs + pill splash. Every page belongs to one archetype:

- **Conversion splash** (`/`, `/california/` giem-13, `/los-angeles/` giem-12, `/where-we-help/` giem-11): photo hero (full 100dvh splash on landers via the chrome block's `main>div:first-child` flex/min-height rules, per Joshua; 100dvh + rotation on the homepage), tabs + pill, Variant A header. `/where-we-help/`'s city cards additionally open a `/value/`-style contact+address gate and hand off to `/value/` for the rendered report (see CLAUDE.md, "City-card handoff").
- **Resource** (`/rates/`, `/prices/`, `/market-insights/`, `/faq/`, `/net-sheet/` (the seller net sheet tool: `#f2f0ef` masthead, a bespoke Places lookup pill + `.ns-gate-*` contact gate on the `/value/` pattern, then the calculator; see CLAUDE.md), and the `/sellers/` + `/buyers/` header-nav hubs (scoped `.hub-*` card grids; `/sellers/` carries the live net-proceeds calculator at `#net-proceeds`, `/buyers/` the closing-cost + monthly estimator at `#closing-costs` with its rate defaulting to the live 30y from `/api/rates`, and `/buyers/` runs its closing CTA and hub buttons in BUY mode via the `tab-buy` tabpanel wrapper; `/homes/` (2026-09-02, `scripts/build_homes.py`): the `#f2f0ef` masthead, then the CRMLS Matrix IDX frame in a scoped `.hm-frame` box (640px tall at base, 760px from `md`, 820px from `lg`, 16px radius, `#e5e5e5` border), the CRMLS Rule 19.2 attribution strip in `.hm-legal` directly under it, a `.hm-steps` three-card band opening the Buy funnel, an `xr--white` band, and a buy-mode closing pill; the only sanctioned iframe on the site, see CLAUDE.md): a `#f2f0ef` masthead band (dark type, symmetric padding via the chrome block) so the opening never reads as a white-on-white void; `/process/` giem-15 and `/field-notes/` giem-17: thin photo band; `/value/`: warm `#efe9e1` tool band with its own address input): NO pill section in the splash, content above the fold, Variant B header. The three data pages carry a contextual `.drozq-inline-cta` card (dark `#1a1816` panel, white copy, red pill; scoped CSS in the chrome block; button calls `window.openFunnel('','sell')`) after their primary data blocks.
- **Trust** (`/about/`: warm flat band into the portrait story; `/meet-the-team/` giem-16 thin band; `/testimonials/`: `.drozq-photo-band` giem-18 strip above the flat hero; `/sold/`: the sold board (giem-19 strip, aggregate stat tiles, image-led `.sold-card` grid with published case-file numbers, one card per closing); `/contact/` giem-14 thin band + direct-contact cards above the fold): no pill in the splash, Variant B header.
- **Utility** (`/privacy/`, `/terms/`, `/thank-you/`, `/404.html`, and `/fello/` (2026-09-02, internal + noindex, `scripts/build_fello.py` scaffolded from `/short-sale/`: `#f2f0ef` masthead, a four-tile `.fl-stats` strip, scoped `.fl-*` card grids / a horizontally scrolling `.fl-table` endpoint reference / a numbered `.fl-list` of limitations / a dark `.fl-dark` CLI band, then an `xr--white` band and the standard sell-mode closing pill; every style scoped in the chrome block, no new Panda utilities)): unchanged layouts, Variant B header. `/thank-you/` keeps its "drop another address" pill (a second-lead play, not a splash CTA).

The persistent lead layer on EVERY page: sticky mobile CTA, header phone, the synced funnel overlay one CTA away, the closing CTA section, the footer. Removing a splash pill never removes the funnel.

Dark-band text on de-photo'd pages uses `fw_800 op_0.87 c_#2b2b2b lh_36px md:lh_44px fs_28px md:fs_36px ls_0.3px mb_16px` (H1) + `c_#3f4650 fs_16px md:fs_18px lh_28px md:lh_32px max-w_640px m_0_auto` (subhead), both compiled. Pages that keep a photo band always carry the 0.4 dark tint overlay (`<div class="pos_absolute top_0 w_100% h_100% z_2" style="background:rgba(26,24,22,0.4)"></div>`), a meaningful `alt`, and `fetchpriority="high"` on the hero image. Giem ledger: 01-10 + 19 + 20 homepage ring, 11 where-we-help, 12 los-angeles, 13 california, 14 contact, 15 process, 16 meet-the-team, 17 field-notes, 18 testimonials.
...
[light gray band #f2f0ef]: "Selling your home can be hard, I make it easy." infographic (heading was "The Hard Parts Are My Job, Not Yours" until 2026-08-26)
...
[band]: brand wall, "Seen by every buyer, everywhere" + one sub line (6 grayscale platform logos)
...
Examples observed:
- "Selling your home can be hard, I make it easy." — 32px / 800
...
- **Brand wall** ("Seen by every buyer, everywhere"). A single centered row of 6 grayscale platform logos (`brand_zillow`, `brand_redfin`, `brand_realtorcom`, `brand_google`, `brand_instagram`, `brand_youtube` in `/media/images/`), real official marks desaturated via CSS `filter: grayscale` at `.6` opacity, and each logo returns to full color + full opacity on `:hover` (a 250ms `filter`/`opacity` transition, Joshua 2026-09-03: "I like the grey, but show their colors on hover"). Container widens to ~1280px on desktop; logos render at 1.3x base. The promise: your listing is seen everywhere buyers look, no marketplace tax.
...
Rules:
- **Two variants, picked against the neighbors:** `xr--warm` (band `#f2f0ef`, cards `#fff` + `#e5e5e5` border) is the default; `xr--white` (band `#fff`, cards `#fbf8f4` + `#ece8e2` border) goes where the closing CTA is white (`/rates/`, `/prices/`, `/market-insights/`, `/testimonials/`, `/thank-you/`). Never white cards on a white band.
- **Cards are contextual, not a generic "related links" list.** Each card names the target page's actual substance with real numbers where they exist (the sold-board numbers: $775,000 Long Beach close with $23,250 in credits; $350,000 Corona close $20,000 off ask; $43,250 total negotiated, 2 for 2 early). When the board grows, sweep the `xr-` bands for stale numbers.
- **Deep anchors are encouraged:** `/sellers/#net-proceeds` and `/buyers/#closing-costs` link straight to the calculators (the sitewide `[id]{scroll-margin-top:80px}` keeps them clear of the fixed header).
- **Inline prose links** use the companion `.xr-a` class (red, bold, underlined): woven where existing copy already names the concept (e.g. `/process/`'s "current submarket data" → `/prices/`, `/sold/`'s "Licensed in January 2026" → `/about/`, two `/faq/` answers → `/testimonials/` + `/prices/`). When linking FAQ answer copy, anchor the replacement on HTML-only boundaries: the same sentences also live inside the FAQPage JSON-LD, which must stay plain text.
- The band sits INSIDE `<main>`, before the closing CTA `<section>` (on case files: before `</main>`; on `/thank-you/`: before `<section class="ty-cta">`). The page still ends on conversion; the band is the second-to-last beat.
...
- **Use an uncompiled Panda utility and assume it works.** The inherited soup only ships classes referenced in `/index.html`. Arbitrary values (`max-w_780px`, `grid-tc_280px_1fr`, `bd_1px_solid_#d92228`, etc.) generate no rule and fail silently. If a new arbitrary value is genuinely needed, add a scoped `<style>` rule for it (as with `.btn-secondary-outline` / `.drozq-portrait-split`); never rely on the class existing. Worst production instance so far: the `/process/` five-steps section shipped as "cards" whose `bd_1px_solid_#e5e5e5`, `bx-sh_0_2px_8px_*`, and `bg-c_#fbf8f4` were all uncompiled, so it rendered as bare circles floating in white space until the 2026-07-14 rebuild (now the scoped `.proc-tl-*` timeline: warm `#efe9e1` panel, red nodes on a rail, white cards, dark total-timeline card). Design intent can die silently: eyeball every new section against what you THINK you styled.

  **The Panda patch (2026-07-14).** A site-wide sweep found 14 more pages rendering degraded designs from this same trap: borderless "card" grids (about, california, contact, meet-the-team, where-we-help, los-angeles, 404), dead `grid-tc_*` layouts collapsing to single columns, mobile heading sizes at desktop (`md:lh_44px` / `md:fs_34px` missing from every migrated soup), invisible accent chips (`bg-c_#fbe9ea` / `bg-c_#fdecec`), dead hovers, dead greens (`c_#0a801f` x29 on /los-angeles/), and overwide text blocks (`max-w_780px`). The fix is `<style id="drozq-panda-patch">` in every page's `<head>` (just before `</head>`): a generated stylesheet compiling exactly the utilities that pages use but no soup defines. Maintained by **`scripts/panda_patch.py`**: `--check` reports any uncompiled visual class on any page (exit 1; part of the new-page checklist), `--apply` regenerates + reinstalls the block site-wide, `--css` prints it. Never hand-edit the patch block; re-run `--apply`. The patch lives outside the funnel markers and is not synced by `sync_funnels.py`.

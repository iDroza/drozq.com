...
- **Conversion splash** (`/`, `/california/` giem-13, `/los-angeles/` giem-12, `/where-we-help/` giem-11): photo hero (70dvh on landers via the chrome block's `main>div:first-child` flex/min-height rules; 100dvh + rotation on the homepage), tabs + pill, Variant A header.
- **Resource** (`/rates/`, `/prices/`, `/market-insights/`, `/faq/`: flat dark compact band; `/process/` giem-15 and `/field-notes/` giem-17: thin photo band; `/value/`: warm `#efe9e1` tool band with its own address input): NO pill section in the splash, content above the fold, Variant B header. The three data pages carry a contextual `.drozq-inline-cta` band (scoped CSS in the chrome block; button calls `window.openFunnel('','sell')`) after their primary data blocks.

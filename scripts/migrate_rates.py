...
(function(){
  ...
  function wireFAQ() {
    var items = document.querySelectorAll('.drozq-faq-item button');
    items.forEach(function(btn){
      btn.addEventListener('click', function(){
        ...
        items.forEach(function(other){
          ...
          var rid = other.getAttribute('aria-controls');
          var region = document.getElementById(rid);
          if (region) region.style.maxHeight = '0';
        });
        if (!expanded) {
          btn.setAttribute('aria-expanded', 'true');
          var rid = btn.getAttribute('aria-controls');
          var region = document.getElementById(rid);
          if (region) region.style.maxHeight = region.scrollHeight + 'px';
        }
      });
    });
  }

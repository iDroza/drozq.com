...
### Fello integration (`/functions/_lib/fello.js`, `/functions/api/fello/*`, 2026-09-02)
...
1. **Outbound push (`pushLeadToFello`, called from `deliverLead`).** Every accepted `/api/lead` submission except newsletter sign-ups, the retired soft-save, and Fello-originated leads (intent or referral starting with "Fello", so the webhook never loops) becomes a Fello contact: email, name, E.164 phone (the `0000000000` placeholder is dropped), the Places-confirmed `full_address`, `crmFields {name: FollowUpBoss, source: drozq.com, stage: Lead, createdDate}`, and the **tag vocabulary** (`felloTagsFor`): always `Drozq Website`; `Seller` / `Buyer` by intent; one mode tag (`Drozq: Sell` / `Drozq: Buy` / `Drozq: Sell + Buy` / `Drozq: Valuation` / `Drozq: Net Sheet` / `Drozq: One Tap` / `Drozq: Lead`); one timeline tag (`Timeline: Now` / `1-3 mo` / `4+ mo` / `Curious`); `Page: <slug>` from `source_page`; `Paid: Google` when a gclid is present. `400 DuplicateContact` means "already nurturing": the tags are appended and the address attached to the existing record. Tags are the only lever into Fello's segments and workflows, so this list is canon; extend it here, in the brief, and on `/fello/` together. Log markers `LEAD_FELLO_PUSHED` / `LEAD_FELLO_DUPLICATE` / `LEAD_FELLO_FAILED` / `LEAD_FELLO_THREW` (masked email).

...
Cloudflare Pages auto-deploys functions from `/functions/`. Seven endpoints currently exist:
...
- **Reviews section rebuilt as the proof block (2026-08-25).** `#home-proof`: "Closed, with the numbers left in." over the $58,250 / 3-for-3-closed-early sub line ("Every one of them was me across the table from a listing agent. Now I'm on your side of it."), three case-file cards with the dollar figure first and a "Read the case file" link into `/testimonials/00X-.../`, then the two named client quotes in a second row. Every star row is gone (the copy principles ban star ratings). The sold board and this block must agree: when a closing lands, update both.

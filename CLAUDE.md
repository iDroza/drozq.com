...
### Returning-visitor identity (durable, 2026-09-04)

`saveReturningData()` / `loadReturningData()` / `populateLandingInputs()` (all synced funnel JS) capture name, email, phone, address and gclid as a visitor fills the funnel (name step, contact step, address pick) and pre-fill the same fields on a later visit. This system used to be localStorage-only (`drozq_funnel_returning`, 30-day TTL) and was silently unreliable on Safari: ITP caps ALL script-writable storage to 7 days regardless of the coded TTL. It's now a hybrid: localStorage stays the fast, same-pageview cache (unchanged), and every `saveReturningData()` call ALSO fire-and-forgets a `POST /api/visitor` (merge semantics; a partial POST from an earlier funnel step never blanks a field a later step didn't resend). On page load, if the local copy came up empty (the common Safari case after 7+ days), the funnel JS falls back to `GET /api/visitor`, hydrates `window.funnelState`, re-runs `populateLandingInputs()` if it recovers an address, and writes the result back into localStorage (`saveReturningData(true)`, skipping the redundant re-POST). `/api/visitor` is keyed by the `drozq_vid` cookie, which `functions/_middleware.js` re-issues via a server-side `Set-Cookie` on every request, so it (and the D1 row it points at) survives past ITP's 7-day cap even when the visitor never touches localStorage again. No PII rides in a client-readable cookie: only the opaque vid does; the actual name/email/phone/address live server-side in D1. Full architecture + the ITP root cause: the "Durable visitor identity" shared guard above. Test: `node scripts/test_visitor.mjs`.
...
Cloudflare Pages auto-deploys functions from `/functions/`. Eight endpoints currently exist, plus the shared guards below.

### Shared guards (`functions/_lib/`, 2026-08-26)

- **Durable visitor identity (`functions/_middleware.js` + `_lib/visitorid.js`, 2026-09-04).** Runs ahead of every Pages request (skipped for static-asset extensions: css/js/images/fonts/etc.) and re-issues two cookies via a `Set-Cookie` RESPONSE header on every visit: `drozq_vid` (an opaque `crypto.randomUUID()`, HttpOnly, 400-day sliding Max-Age, no PII) and `gclid` (unchanged shape, JS-readable, 90-day sliding Max-Age; a fresh `?gclid=` query param always wins over a stored value). **Why a middleware, not just the existing client-side `document.cookie` / `localStorage` code:** Safari ITP caps ALL script-writable storage, cookies AND localStorage alike, to 7 days of no direct top-level, interacted visit, no matter what Max-Age or TTL the JS codes. A paid-search visitor who clicks an ad, doesn't convert same session, and returns 2+ weeks later on mobile Safari (most of this site's traffic, per the mobile-primary rule) was silently losing both gclid attribution and returning-visitor recognition, while a server-rendered site (cookies set via HTTP `Set-Cookie`, which ITP does NOT cap) does not have this problem. Re-issuing the cookie via a response header on every visit, not via JS, is the fix: it never depends on the visitor coming back inside 7 days. The middleware itself does no D1 work and is safe to run on every request; on any failure it falls back to the untouched original response rather than ever breaking a page load. See "Returning-visitor identity" under the funnel architecture section for the client-side half.
...
### Fello integration (`/functions/_lib/fello.js`, `/functions/api/fello/*`, 2026-09-02): SWITCHED OFF the same evening

**STATUS: OFF, in full, by Joshua's order (2026-09-02, "turn everything regarding fello off").** Nothing on drozq.com talks to Fello in either direction: no lead push (removed from `/api/lead`), no webhook subscriptions (all ten unsubscribed in Fello), no dashboard readback (the Worker's `DROZQ_EMAIL_SECRET` is deleted, so `sources/fello.ts` reports `unconfigured` and the "Fello Nurture" cards were removed from `/dashboard/`). The code below is dormant and gated: the Pages secrets `FELLO_API_KEY` + `FELLO_CLIENT_SECRET` are KEPT in Cloudflare and in the gitignored `scripts/.fello_secret` (Joshua: "keep the keys"), so `/api/fello/webhook` would still verify a signed event if one arrived, and `/api/fello/engagement` still answers an admin call; neither is triggered by anything. Turning it back on is Joshua's explicit call, and even then the lead push stays off (see [[feedback-never-push-leads-to-fello]] in memory). The paragraphs that follow describe the dormant build.

Fello (fello.ai) is the home-value dashboard + seller-nurture platform Joshua bought 2026-09-02. Its public API is a contact-sync API (12 endpoints, 10 webhook events, no list/search/export, **no home values**: the AVM / equity / mortgage attributes only leave Fello through the native Fello -> Follow Up Boss sync, as FUB custom fields). Brief: `notes/fello/fello-api-brief.md`, rendered at `/fello/`. CLI: `scripts/fello.py` (`probe`, `webhooks`, `contact`, `tags`, `property`, `verify`, `calllist`). Credentials: gitignored `scripts/.fello_secret`; Pages env `FELLO_API_KEY` (the Custom App key, `x-api-key`) + `FELLO_CLIENT_SECRET` (webhook HMAC only); the dashboard Worker holds `DROZQ_EMAIL_SECRET` (= `EMAIL_SECRET`) to read the engagement endpoint. Everything is gated on those vars and byte-identical without them. Tests: `node scripts/test_fello.mjs` (46 checks, mocked fetch + the D1 stand-in).

1. **Outbound push: OFF, permanently, by Joshua's order (2026-09-02, "do not fucking submit leads to fello").** `/api/lead` never calls `pushLeadToFello`; the site sends NOTHING to Fello. The function and the tag vocabulary (`felloTagsFor`: `Drozq Website`, `Seller` / `Buyer`, `Drozq: <mode>`, `Timeline: <bucket>`, `Page: <slug>`, `Paid: Google`) stay in `_lib/fello.js` for the CLI (`fello.py contact add`) and a future explicit opt-in only. Never re-add the push to the lead pipeline without Joshua saying so in writing. `scripts/test_fello.mjs` asserts a lead with `FELLO_API_KEY` set makes zero Fello calls. The leads pushed during the ~40 minutes it was live were deleted from Fello the same evening.
2. **Webhook receiver (`POST /api/fello/webhook`).** 503 until `FELLO_CLIENT_SECRET` exists, 401 on a bad `fello-webhook-signature` (HMAC-SHA256 of the raw body keyed by the base64-decoded secret, constant-time compare), 200 immediately, all work behind `waitUntil`, each event deduped 6 h on a SHA-1 of the event via the idempotency store, each captured to PostHog as `fello_<snake_type>`. Routing: `FormSubmission` -> an internal form POST to `/api/lead` (intent `Fello Seller Lead: <leadType>` or `Fello Buyer Lead: ...`, `referral_source=Fello`, `source_page=fello:<sourceType>:<sourceDetail>`, the leftover form fields in the message, `submission_id=fello-<hash>`; the lead handler then alerts, pushes FUB, enrolls the drip, and skips the Fello push); `DashboardClick` / `EmailClick` / `PostcardScan` / `FelixAIHandoff` -> the HOT path: Fello contact read, a branded alert to `TO_EMAIL` ("🔥 Fello hot: <name> ...", score / phone / property / signals, tap-to-call), and a FUB `General Inquiry` event tagged `Fello` + `Drozq Hot` (+ `Fello Handoff`); `ContactUnsubscribed` -> the drozq drip subscriber is paused (never FUB, never re-subscribed); `ContactEnriched` / `ContactDetailsUpdated` -> the FUB person is PATCHed (name / phone / Fello signal tags such as `FELLO HIGH EQUITY`) with no timeline event; `TagsAdded` -> tags mirrored onto the FUB person; `TagsRemoved` / unknown -> logged. Markers `FELLO_WEBHOOK_*`. Register with `python scripts/fello.py webhooks add-all https://drozq.com/api/fello/webhook` (3 subscriptions per event max; Fello auto-unsubscribes a failing URL).
3. **Engagement readback (`GET /api/fello/engagement`, Bearer `EMAIL_SECRET`).** Sweeps the newest 100 non-newsletter leads from D1 `subscribers` (last 90 days) through Fello's contact read (5 concurrent), ranks hot (a dashboard/email CTA click inside 7 days) > warm (a view) > newest activity > lead score, decorates hot + warm leads with the Fello-fed custom fields on their FUB person (`custom*` keys matching value / equity / mortgage / score / Fello, whatever the native sync was mapped to write), and caches the result 8 minutes in D1 `fello_engagement_cache`. `?days= ?limit= ?hotDays= ?fresh=1`. The dashboard Worker's `sources/fello.ts` reads it every cron and publishes `felloHotLeads7d`, `felloLeadsScored`, `felloAvgLeadScore` (a "Fello Nurture" section on `/dashboard/`; numbers only, the page is public). `python scripts/fello.py calllist [--csv]` prints the named list.
...
### `/functions/api/visitor.js` (2026-09-04)

The D1-backed half of the durable returning-visitor system (see "Durable visitor identity" and "Returning-visitor identity" above for the full why). Keyed by the `drozq_vid` cookie `functions/_middleware.js` re-issues on every request; never trusts a client-supplied id.

- `GET`: no vid cookie, no D1 row, or a row whose `updated_at` is older than 30 days -> `{ok:true, found:false}`. Otherwise `{ok:true, found:true, data:{fullName, email, phone, address, gclid, updatedAt}}`.
- `POST`: form or JSON body with any of `fullName`/`email`/`phone`/`gclid`/`address` (address as a JSON string or object carrying `.formatted`). Merges into the visitor's row via one `INSERT ... ON CONFLICT DO UPDATE` with a per-column `CASE WHEN <new> != '' THEN <new> ELSE <existing> END`, so a step that only has the phone (the funnel captures name, then email+phone, then address, across separate steps) never blanks a field an earlier step already wrote. No vid cookie -> soft `{ok:false, error:"no_visitor_id"}` at 200 (never a hard failure the client has to handle). Rate-limited at 30/IP/10min (`visitor:10m`, not part of `RATE_RULES` since it's not money-spend like Rentcast, just D1-write hygiene). No email is sent, no CRM record touched, no honeypot, no consent gate: this is a client-experience cache, not a lead.
- Table: D1 `returning_visitors(vid PRIMARY KEY, full_name, email, phone, address_json, gclid, updated_at)` via `EMAIL_DB` (same binding as the rate-limit/idempotency/email tables), created lazily. A 2% chance on each POST prunes rows older than 60 days (buffer past the 30-day TTL) via `waitUntil`.
- Unconfigured (`EMAIL_DB` unbound): `GET` -> `found:false`, `POST` -> `503 unconfigured`. Byte-identical degrade to every other D1-backed endpoint on the site.
- **Telemetry:** the endpoint had none otherwise, so every call fires a server-side `phCapture()` (reusing `_lib/email.js`, behind `waitUntil`) with `distinct_id` = the vid: `visitor_id_get` (`found` bool, `reason` when false: `unconfigured`/`no_vid`/`no_row`/`expired`/`error`, plus `age_days` and the `has_*` field flags when found) and `visitor_id_post` (`outcome`: `saved`/`no_vid`/`rate_limited`/`unconfigured`/`empty`/`error`, plus `has_name`/`has_email`/`has_phone`/`has_address`/`has_gclid` when saved). Query in PostHog: `SELECT properties.reason, count() FROM events WHERE event = 'visitor_id_get' AND properties.found = false GROUP BY 1` to watch for `no_vid` spiking (would mean the middleware isn't setting the cookie for some traffic slice) or `visitor_id_get` where `found=true` trending up over the days after 2026-09-04 launch (the fix working: visitors getting recognized past what localStorage alone would have survived).
- Test: `node scripts/test_visitor.mjs`.
...
- **Homepage type scale (2026-09-03, Joshua: "I like the size of these [the stat bar] ... resize the other text to match this ... not a huge fan of the small text").** Every block below the splash now reads at `#drozq-commit-strip`'s scale via one scoped block, `<style id="drozq-home-type-css">` in `<head>`, keyed by section id (`#home-steps`, `#home-hard`, `#home-market`, `#home-proof`, `#home-condition`, `#home-five-steps`, `#what-i-owe-you`, `#brand-wall`, `#home-faq`, `#home-closer`): section h2s `clamp(30px,3.4vw,46px)`, sub-headlines at the kicker's `clamp(20px,2.4vw,27px)`, body 18/28 (19/30 at `lg`), card copy 17/26 (18/28), uppercase labels 12.5/13px, the proof-card dollar figures at `clamp(30px,3.2vw,44px)` with the label as a block beneath, the market widget's two figures with their red uppercase label UNDER the number like the strip. Full table + the layout knock-ons (Hard Parts item widths, the condition section's fixed `lg:h_800px` relaxed to a min-height) in `TEMPLATE.md` §5 "Homepage type scale". Homepage only; the sibling-page rollout is a BACKLOG item awaiting Joshua. The splash, the synced funnel, header and footer are untouched. Same pass caught the homepage carrying the retired 7-day launch line in four places (the Move-in ready card, both "What I owe you" renders, Five Steps step 04) while eleven pages already said 48 hours; all four now carry the canon.
...
- `scripts/fello.py` (2026-09-02): the Fello API CLI (`probe`, `webhooks list|add|add-all|rm`, `contact get|add|update|delete`, `tags add|replace|rm`, `property add|archive`, `verify` for the webhook HMAC). Reads the gitignored `scripts/.fello_secret` (JSON `api_key` + `client_secret`; rotate in Fello > Settings > Connected Apps). The brief of what the API can and cannot do is `notes/fello/fello-api-brief.md`, rendered at `/fello/`; `scripts/build_fello.py` rebuilds the page. The integration shipped 2026-09-02 (see "Fello integration" under Cloudflare Pages Functions): `deliverLead` pushes every real lead into Fello, `/api/fello/webhook` receives Fello's events, `/api/fello/engagement` ranks leads by Fello engagement for the dashboard + `fello.py calllist`.

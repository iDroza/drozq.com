...
**Lead-capture gate (2026-06-24).** When a valuation succeeds, the numbers do NOT render immediately. The page holds the data and shows a modal (`#value-gate`, scoped `.value-gate-*` classes, page-specific JS, NOT synced) titled "Where should I send it?" that collects name + email + phone. It is a **hard gate: submitting the form is the only path to the numbers.** Closing the popup (the X or the Esc key, via `abandonGate`) discards the held valuation, resets `lastQuery` (so the same address re-runs and re-gates if entered again), and returns to the address form **without ever revealing the numbers**; backdrop clicks do nothing (so an accidental tap-outside can't dump the visitor out of the required form), and there is no skip link. The address is still soft-saved (placeholder identity, `intent="Home Valuation View"`, not pushed to FUB) the instant the valuation succeeds, so even an abandoned gate captures the address as the safety net. A real submit posts a richer lead to `/api/lead` with `intent="Home Valuation Lead"` (matches `/valuation/` so it maps to FollowUpBoss "Seller Inquiry"; distinct from the `"Home Valuation View"` soft-save string, so it IS pushed to FUB) carrying the real contact + full address components + gclid + TCPA consent, then reveals the numbers instantly without waiting on that POST. Phone uses the same `normalizeUsDigits` country-code strip as the funnel. PostHog/dataLayer events: `valuation_gate_shown` / `valuation_gate_submit` / `valuation_gate_dismiss` (the last carries `method`: close / esc). Copy says "I" not "we" per the solo-agent voice rule. The gate sits at z-index 12000 (above the funnel overlay's 9999).

...
|---|---|---|---|---|
| Sell | `sell` | 5 | "Send My Report" | `Home Valuation` |
| Buy | `buy` | 5 | "Send My Report" | `Home Purchase` |
...
- **Open follow-up:** the "instant" copy currently runs ahead of the backend (no auto-delivery of the report yet). The legacy per-mode `VALUEBAR.buy/sellandbuy` + `DELIVERABLE.buy/sellandbuy` + `DV_BODY` + `.funnel-dv-*` CSS are now dead (see `BACKLOG.md`).

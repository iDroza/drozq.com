...
- Drozq social URLs (footer row order, 2026-07-22): Zillow → `zillow.com/profile/ImJoshua`, Facebook → `facebook.com/Drozq/`, Instagram → `instagram.com/drozq/` (center), YouTube → `youtube.com/@drozq`, LinkedIn → `linkedin.com/in/guerrero-joshua/` (last). All five also live in every page's JSON-LD `sameAs`. The Zillow icon is the official Z-house swoosh mark (geometry traced from `s.zillowstatic.com/pfs/static/z-logo-icon.svg`, the two ™ subpaths dropped, scaled into the shared 20×20 white-disc evenodd-cutout style; glyph subpath starts `M11.83 7.37`).

...
**Lead-capture gate (2026-06-24).** When a valuation succeeds, the numbers do NOT render immediately. The page holds the data and shows a dismissible modal (`#value-gate`, scoped `.value-gate-*` classes, page-specific JS, NOT synced) titled "Where should I send it?" that collects name + email + phone before revealing the 5 systems. It is deliberately easy to escape: an X button, a backdrop tap, the Esc key, and an explicit "Skip, just show me the numbers" link all close it, and **dismissing still reveals the numbers** (the address was already soft-saved as the safety net above, so a lead is never lost). A real submit posts a richer lead to `/api/lead` with `intent="Home Valuation Lead"` (matches `/valuation/` so it maps to FollowUpBoss "Seller Inquiry"; distinct from the `"Home Valuation View"` soft-save string, so it IS pushed to FUB) carrying the real contact + full address components + gclid + TCPA consent, then reveals the numbers instantly without waiting on that POST. Phone uses the same `normalizeUsDigits` country-code strip as the funnel. New PostHog/dataLayer events: `valuation_gate_shown` / `valuation_gate_submit` / `valuation_gate_dismiss` (the last carries `method`: close / backdrop / esc / skip). Copy says "I" not "we" per the solo-agent voice rule. The gate sits at z-index 12000 (above the funnel overlay's 9999).
